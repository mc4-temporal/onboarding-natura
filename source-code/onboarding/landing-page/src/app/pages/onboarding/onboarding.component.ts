...
export class OnboardingComponent implements OnInit {
  ...
  submitInfoForm() {
    if (FormUtil.isValidFormGroup(this.pInfoForm)) {
      ...
      if (body.tipoDocumento !== 'CI') {
        body.complemento = null;
      }
      this.landingService.requestSaveOnboardingInfo(body)
  ...
  protected successSaveInfo = (body: any) => {
    if (!this.existeFlujoPendiente(body)) {
      this.responseInfo = body;
      Notiflix.Loading.remove();
      this.stepper.next();
    }
  ...
  protected existeFlujoPendiente(body: any) {
    return false;
  }
}
